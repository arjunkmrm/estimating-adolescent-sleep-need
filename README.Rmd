...
```{r, echo=FALSE, message=FALSE, fig.align="center", fig.width=8, fig.height=8}
...
```{r, message=FALSE, fig.align="center", fig.width=8, fig.height=8}
...
#extract critical wake duration
crit <- data.frame()
crit <- sleep.allconditions.lapses$coefficients$random$subj + sleep.allconditions.lapses$coefficients$fixed[2]
crit <- data.frame(crit)
...
ggplot(crit, aes(x = crit, y = id)) +
  geom_point(alpha = 0.5, size = 0.8, color = "darkgreen") +
  geom_vline(xintercept = sleep.allconditions.lapses$coefficients$fixed[2], color = "navyblue", linetype = 2, size = 0.7) +
  labs(x = "critical waking duration", y = "id")
```
...
plot(sleep.allconditions.lapses, cex=0.5)
...
ggplot(crit, aes(x = crit)) +
  #stat_function(fun = dnorm, args = list(15.88, 1.42), color = "red", size = 1, alpha = 0.5) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = sleep.allconditions.lapses$coefficients$fixed[2], sd = 0.9), aes(fill = "NFS, age: 15 - 19"), alpha = 0.2) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = 15.82, sd = 3.84), aes(fill = "Van Dongen et al., age: 21 - 38"), alpha = 0.2)+
  geom_area(stat = "function", fun = dnorm, args = list(mean = 14.65, sd = 4.14), aes(fill = "Short et al., age: 15 - 17"),  alpha = 0.2)
...
#graphing lapses vs excess
crit.lapses <- sleep.allconditions.lapses$coefficients$random$subj + sleep.allconditions.lapses$coefficients$fixed[2]
crit.lapses <- data.frame(crit.lapses)
