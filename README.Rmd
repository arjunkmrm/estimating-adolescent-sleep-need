...
The dataset for this analysis comes from NFS1, NFS2, NFS4 and NFS5. Here is a section of the original dataset. Most of the columns have been ommitted for the sake of presentation here.
...
data.nfs = data.nfs %>% na.omit() #remove na
...
```{r, echo = FALSE, message=FALSE, fig.height = 6, fig.width=6, dpi=300}
...
sdcrit <- intervals(st.nonap.lapses)
...
ggplot(crit, aes(x = crit)) +
  ...
  geom_area(stat = "function", fun = dnorm, args = list(mean = st.nonap.lapses$coefficients$fixed[3], sd = sdcrit), aes(fill = "NFS, age: 15 - 19"), alpha = 0.2) +
...
#lapses predictions
data.nonap$plapses = predict(st.nonap.lapses, data.nonap)
data.nonap$plapses[is.nan(data.nonap$plapses)] <- 0.0

#graphing lapses vs excess
crit.lapses <- st.nonap.lapses$coefficients$random$subj + st.nonap.lapses$coefficients$fixed[3]
crit.lapses <- data.frame(crit.lapses)
